...
void Solver::sum_adjacent_numbers(const size_t& i, const size_t& j)
{
    ...
    size_t to_add = 1, extracted_numbers = 0;
    for(const std::pair<size_t, size_t>& index : indexes)
    {
        ...
        to_add *= extracted_number;
        extracted_numbers++;

        if(extracted_numbers > 2)
        {
            this->log("Found more than 2 adjacent numbers, not a valid gear.");
            break;
        } 
    }
    ...
    if(extracted_numbers == 2)
    {
        this->log("Found gear, adding " + std::to_string(to_add) + " to the answer");
        this->answer += to_add;
    }
}
